...
#############################################################*/
var selects1= document.getElementsByTagName('select');
var j=0
var selects=[];
//----------------修改专用-------------------------
var sname="";
...
for(var i=0;i<selects1.length;i++){
	if(selects1[i].parentNode.className=="select"||selects1[i].parentNode.className=="eselect"){
		
	}else{
		selects[j]=selects1[i];
		j++;
	}
}
...
function rOptions(i, name) {
	...
	for (n=0;n<selects[i].options.length;n++){	
		...
		if(option_selected){
			...
			applySelect(option_li.innerHTML);
		}
...
function applySelect(sel_op){
	if(sel_op == "电信南方区"){
		document.getElementById('dQ').value="1500200";
	}else if(sel_op == "联通北方区"){
		document.getElementById('dQ').value="1500100";
	}else{
...
window.onload = function(e) {
	...
	bodyclick.onclick = function(){
		for (i=0;i<selects.length;i++){	
		if(selects[i].parentNode.className!="select"){
			document.getElementById('select_info_' + selects[i].name).className = 'tag_select';
			document.getElementById('options_' + selects[i].name).style.display = 'none';}
		}
	}
